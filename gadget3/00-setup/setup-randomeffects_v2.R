...
random_actions <-
  c(
    ...
    if (random_initial){
      if (random_initial_randomwalk){
        list(
          g3l_random_walk('initialconditions',
                          substitute(x,
                                     list(x = 
                                            {if (!exponentiate_init){
                                              substitute(log(avoid_zero(x2)),
                                                         list(x2 = 
                                                                g3_parameterized(paste0(
                                                                  if (single_stock_model) gadgetutils::g3_stock_name(single_stock)
                                                                  else gadgetutils::g3_stock_name(imm_stock, 'species'), '.init',
                                                                  ifelse(exponentiate_init, '_exp', '')
                                                                ),
                                                                by_year = TRUE,
                                                                random = penalise_recruitment == 0)))
                                            }else{
  ...
  #list(
    if (random_M){
      year_range %>%  
        map(function(yr) 
          g3l_random_dnorm(sprintf('naturalmortality_yr%s',yr),
                           substitute(log(avoid_zero(x)), 
                                      list(x = 
                                             g3_parameterized(sprintf('bling.M.%s',yr),
                                                              random = penalise_M == 0))),
                           mean_f = g3_parameterized('zero', scale = 0),
                           #g3_parameterized('bling_M_fixed', optimise = FALSE, value = 0.15),
                           weight = g3_parameterized('rnd_M_weight', scale = -1),
                           sigma_f = g3_parameterized('bling_M_sigma')))  
    } else NULL  
  #)
  ,
  list()
